...
def define_common_args(parser : argparse.ArgumentParser):
    ...
    group.add_argument('--grey_bkgd',
                         type=bool,
                         default=False,
                         help="Whether to use grey background (ignored in some datasets)")
    group.add_argument('--llffhold',
                         type=int,
                         default=8,
                         help="LLFF holdout every")
    group.add_argument('--depth_source',
                         type=str,
                         default='',
                         help="Where to get depths")
    group.add_argument('--use_zdepth',
                         type=bool,
                         default=False,
                         help="Where to use zdepth from VGGT.")
    group.add_argument('--use_vggt_intri',
                         type=bool,
                         default=False,
                         help="Where to use intrinsics from VGGT.")
...
def build_data_options(args):
    ...
    return {
        ...
        'seq_id': args.seq_id, # for CO3D only
        'epoch_size': args.epoch_size * args.__dict__.get('batch_size', 5000),
        'scene_scale': args.scene_scale, # scale scenes globally by putting cameras near or far
        'scale': args.scale, # downloading images (change intrinsics of cameras)
        'white_bkgd': args.white_bkgd,
        'grey_bkgd': args.grey_bkgd,
        'hold_every': args.llffhold,
        'normalize_by_bbox': args.normalize_by_bbox, # NSVF dataset only
        'data_bbox_scale': args.data_bbox_scale, # NSVF dataset only
        'cam_scale_factor': args.cam_scale_factor, # NSVF/CO3D dataset only
        'normalize_by_camera': args.normalize_by_camera, # NSVF dataset only
