...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    if not os.path.isfile(os.path.join(basedir, "hwf_cxcy.npy")):
        intrinsic_arr = poses[:, 4, 0] # (H, W, focal)
        poses = poses[:, :4, :]

    '''
    if os.path.isfile(os.path.join(basedir, "vggt_intrinsic.txt")):
        fp = open(os.path.join(basedir, "vggt_intrinsic.txt"), 'r')
        fxy = fp.read().strip().split(',')
        assert fxy[0].strip().split(' ')[0] == 'fx'
        fx = float(fxy[0].strip().split(' ')[1])
        assert fxy[1].strip().split(' ')[0] == 'fy'      
        fy = float(fxy[1].strip().split(' ')[1])
        fp.close()
        intrinsic_arr = np.array([intrinsic_arr[0], intrinsic_arr[1], fx, fy, intrinsic_arr[1]/2.0, intrinsic_arr[0]/2.0])
    '''
    ...
    with open(os.path.join(basedir, 'extrinsics_c2w_llff.txt'), 'w') as fp:
        for i, (imgfile, extrinsic) in enumerate(zip(imgfiles, poses.transpose([2,0,1])[:,:3,:4])):
            extrinsic = extrinsic.tolist()
            fp.write(imgfile.split("\\")[-1]+f'({i})'+':'+'\n')
            for row in extrinsic:
                for c in row:
                    fp.write(f'{c:-12.4}  ')
                fp.write('\n')

    with open(os.path.join(basedir, 'extrinsics_w2c_opencv.txt'), 'w') as fp:
        for i, (imgfile, extrinsic) in enumerate(zip(imgfiles, poses.transpose([2,0,1])[:,:3,:4])):
            extrinsic = np.concatenate([extrinsic[:,1:2],extrinsic[:,0:1],-extrinsic[:,2:3],extrinsic[:,3:4]], axis=-1)
            R = extrinsic[:,:3]
            T = extrinsic[:,-1:]
            extrinsic = np.concatenate([R.T, -R.T@T], axis=-1)
            extrinsic = extrinsic.tolist()
            fp.write(imgfile.split("\\")[-1]+f'({i})'+':'+'\n')
            for row in extrinsic:
                for c in row:
    ...
    with open(os.path.join(basedir, 'bds.txt'), 'w') as fp:
        for i, imgfile in enumerate(imgfiles):
            fp.write(imgfile.split("\\")[-1]+f'({i})'+': '+f'{bds[0,i]/10:.3f}m, {bds[1,i]/10:.3f}m\n')

    with open(os.path.join(basedir, 'intrinsic.txt'), 'w') as fp:
        if len(intrinsic_arr) == 3:
            H, W, focal = intrinsic_arr.tolist()
            fp.write(f"{H/4}, {W/4}, {focal/4}") # 4x downsampling defaultly
...
def load_llff_data(
    ...
    poses, bds, intrinsic = _load_data(
        basedir, factor=factor, load_imgs=False # Note that factor is None, meaning no downsampling
    )  # factor=8 downsamples original imgs by 8x

    print("Loaded LLFF data", basedir, bds.min(), bds.max()) # 单位是分米 # bds是绝对深度还是z-深度

    # Correct rotation matrix ordering and move variable dim to axis 0
    # poses [R | T] [3, 4, images]
    # LLFF --> OpenGL
    poses = np.concatenate([poses[:, 1:2, :], -poses[:, 0:1, :], poses[:, 2:, :]], 1) # 变换坐标系
    ...
    if recenter:
        poses = recenter_poses(poses) # 作用是改变世界坐标系的位置
        # util.myutils.vis_cams(poses[:1], intrinsic,'center')
    ...
    validation_ids[::split_train_val] = -1   # not for sparse views
    ...
    reference_view_id = np.argmin(dists)  # 原点距离平均位姿的原点最小
    reference_depth = train_bds[reference_view_id]
    print(reference_depth)

    return (
        reference_depth,
        reference_view_id, # for all training views
        render_poses,
        poses,
        intrinsic,
        sc
