...
class SparseGrid(nn.Module):
    ...
    def __init__(
        ...
        alpha_c: bool=False,
        use_alpha_prune: bool=False,
        use_extra_alpha_prune: bool=False,
        use_depth_prune: bool=False,
        d_prop: float=0.99,
        unrobust_d_pro: float=0.8,
        use_isotropy_init: bool=False,
        density_init_withDepth: float=128.0,
        ...
        self._offset = 0.5 * (1.0 - self.center / self.radius) # scenes located in [-1,1]^3 for nerf-synthetic
        self._scaling = 0.5 / self.radius # tranform points within rectangel centered at self.center with self.radius into [0,1]^3 using self._offset and self._scaling
        ...
        mask = (torch.ones(n3) == 1.0).to(device)
        if alpha_c and use_alpha_prune:
            free_space = (rays.alphas == 0.0).squeeze()
            origins = rays.origins[free_space].to(device)
            dirs = rays.dirs[free_space].to(device)
            if use_extra_alpha_prune:
                assert rays.prune_origins != None
                assert rays.prune_dirs != None
                assert len(rays.prune_origins) == len(rays.prune_dirs)
                prune_origins = rays.prune_origins.to(device)
                prune_dirs = rays.prune_dirs.to(device)
                origins = torch.concat([origins, prune_origins], dim=0)
                dirs = torch.concat([dirs, prune_dirs], dim=0)
            start = time.time()
            alpha_mask = self._alphas_prune(Rays(origins, dirs), torch.asarray(reso, device='cpu')).reshape(-1)
            print(f'Elapsed time {time.time()-start:.2f}s for alpha prune')
            mask = torch.logical_and(mask, alpha_mask)

        if use_depth_prune:
            if rays.masks != None:
                masks = rays.masks.squeeze()
                origins = rays.origins[masks].to(device)
                dirs = rays.dirs[masks].to(device)
                depths = rays.depths[masks].to(device)
                gt = rays.gt[masks].to(device)
                if alpha_c:
                    masks_unrobust = torch.logical_and((rays.alphas==1.0).squeeze(), ~masks)
                else:
                    masks_unrobust = ~masks
                origins_unrobust = rays.origins[masks_unrobust].to(device)
                dirs_unrobust = rays.dirs[masks_unrobust].to(device)
                depths_unrobust = rays.depths[masks_unrobust].to(device)
                gt_unrobust = rays.gt[masks_unrobust].to(device)
                assert depths_unrobust != None, "Unrobust depth information is not acquired."
                if torch.all(depths_unrobust>0.0) and dataset_type == "nerf":
                    start = time.time()
                    depth_mask_unrobust, _, _, _ = self._depth_prune(Rays(origins_unrobust, dirs_unrobust), depths_unrobust, unrobust_d_pro, gt_unrobust, 0.1, torch.asarray(reso, device='cpu'), robust_depth=False)
                    print(f'Elapsed time \033[96m{time.time()-start:.2f}s\033[0m for unrobust depth prune')
                    mask = torch.logical_and(mask, depth_mask_unrobust.reshape(-1))
            else:
                origins = rays.origins.to(device)
                dirs = rays.dirs.to(device)
                depths = rays.depths.to(device)
                gt = rays.gt.to(device)
            assert depths != None, "Depth information is not acquired."
            start = time.time()
            depth_mask, grid_mask_isotropyInit, grid_density_init, grid_color_isotropyInit = self._depth_prune(Rays(origins, dirs), depths, d_prop, gt, density_init_withDepth, torch.asarray(reso, device='cpu'))
            print(f'Elapsed time \033[96m{time.time()-start:.2f}s\033[0m for robust depth prune')
            mask = torch.logical_and(mask, depth_mask.reshape(-1))
        ...
        if use_sphere_bound:
            ...
            roffset = 1.0 / gsz - 1.0
            rscaling = 2.0 / gsz
            # mapping grid frame [0, gsz] to world frame [-1, 1]
            ...
            # voxel diagonal length
            sphere_bound_mask = norms <= 1.0 + (3 ** 0.5) / gsz.max()
            mask = torch.logical_and(mask, sphere_bound_mask)
        ...
        if mask.sum() < n3:
            ...
            # init_links中存储的是相对指针（相对于数据表的起始位置），即索引号
        ...
        self.density_data.data[:] = init_sigma
        ...
        # self.sh_data.data[:] = 0.2
        # self.sh_data.data[:] = 0.0

        if use_isotropy_init:
            grid_mask_isotropyInit = torch.logical_and(grid_mask_isotropyInit.reshape(-1), mask) # 确保是有效体素
            ids = init_links[grid_mask_isotropyInit].to(torch.long)
            self.density_data.data[ids] = grid_density_init.reshape(-1,1)[grid_mask_isotropyInit]
            self.sh_data.data[ids,0] = (grid_color_isotropyInit.reshape(-1,3)[grid_mask_isotropyInit,0]-0.5)/utils.SH_C0
            self.sh_data.data[ids,self.basis_dim*1] = (grid_color_isotropyInit.reshape(-1,3)[grid_mask_isotropyInit,1]-0.5)/utils.SH_C0
            self.sh_data.data[ids,self.basis_dim*2] = (grid_color_isotropyInit.reshape(-1,3)[grid_mask_isotropyInit,2]-0.5)/utils.SH_C0
        ...
        self.reso = reso
        self.init_links = init_links.view(reso)
        self.cinit_links = torch.asarray(self.init_links, copy=True)
        self.register_buffer("links", self.init_links)
        ...
        self.acc = acc
        if self.acc and self.links.is_cuda and use_sphere_bound:
            self.accelerate()

        self.use_gradient_scaling = use_gradient_scaling
        if use_gradient_scaling:
            self.gs_grid = self._get_gs_grid(reso, dataset_type, device)
    ...
    def _get_gs_grid(self, reso, dataset_type, device):
        if dataset_type == "nerf":
            assert reso[0] == reso[1] == reso[2]
            assert reso[0] % 2 == 0 
            pad_list = torch.linspace(2, 1, reso[0]//2)**2
            gs_grid = torch.ones(2,2,2) * pad_list[0]
            p3d = (1,1,1,1,1,1)
            for pad_value in pad_list[1:]:
                gs_grid = F.pad(gs_grid, p3d, "constant", pad_value)
        elif dataset_type == "llff":
            # pad_list = torch.ones(reso[-1]) * (1 / math.exp(-0.1*(1/reso[-1])))
            # pad_list[0] = 1.
            # pad_list = torch.cumprod(pad_list, dim=0)
            # gs_grid = torch.ones(reso[0], reso[1], reso[2])
            # for i, pad_value in enumerate(pad_list):
                # gs_grid[..., i] *= pad_value
            sigma_grid = torch.zeros(reso[0], reso[1], reso[2])
            sigma_grid[self.links>=0] = self.density_data.data[self.links[self.links>=0].to(torch.long)].squeeze().cpu()
            sigma_grid[sigma_grid<0] = 0.0
            gs_grid = torch.cumsum(sigma_grid, dim=-1)
            gs_grid_tran = torch.exp(-gs_grid*(1/(self._scaling[-1]*reso[-1])))
            # gs_grid_alpha = 1. - torch.exp(-sigma_grid*(1/(self._scaling[-1]*reso[-1])))
        ...
        gs_grid = gs_grid.to(device)

        return gs_grid

    def _fetch_links(self, links):
        # all sigma values and sh coffs are zero in pruned voxels
    ...
    def _alphas_prune(self, rays: Rays, reso: torch.Tensor):
        origins = self.world2grid(rays.origins, gsz=reso)
        ...
        grid_mask = torch.ones((gsz[0],gsz[1],gsz[2]), device=origins.device) == 1.0
        ...
        while good_indices.numel() > 0:
            ...
            # l[:, 0] = torch.clamp_max(l[:, 0], (gsz_cu[0] - 2).to(torch.long))
            # l[:, 1] = torch.clamp_max(l[:, 1], (gsz_cu[1] - 2).to(torch.long))
            # l[:, 2] = torch.clamp_max(l[:, 2], (gsz_cu[2] - 2).to(torch.long))
            # pos -= l

            lx, ly, lz = l.unbind(-1)
            grid_mask[lx, ly, lz] = False
    ...
    def _depth_prune(self, rays: Rays, depths: torch.Tensor, d_prop: float, gt: torch.Tensor, density_init_withDepth: float, reso: torch.Tensor, robust_depth=True):
        origins = self.world2grid(rays.origins, gsz=reso)
        dirs = rays.dirs / torch.norm(rays.dirs, dim=-1, keepdim=True)
        B = dirs.size(0)
        assert origins.size(0) == B
        gsz = reso
        # t_d_old = ((depths * dirs) * (self._scaling * gsz).to(device=dirs.device)).norm(dim=1) * d_prop # tranform depth from world coordinates to grid coordinates
        t_d = (depths.squeeze() * (dirs * (self._scaling * gsz).to(device=dirs.device)).norm(dim=1)) * d_prop
        dirs = dirs * (self._scaling * gsz).to(device=dirs.device) # transform directions from world coordinates to grid coordinates # (self._scaling * gsz)表示世界坐标中1个单位长度对应的网格坐标中的长度
        delta_scale = 1.0 / dirs.norm(dim=1) 
        dirs *= delta_scale.unsqueeze(-1) # normalize directions in grid coordinate frame
        dirs[dirs == 0] = 1e-9
        invdirs = 1.0 / dirs 
        assert not torch.isinf(invdirs).any() and not torch.isnan(invdirs).any(), "ineffective values!"
        ...
        tmax = torch.max(t1, t2)
        tmax[dirs == 0] = 1e9
        tmax = torch.min(tmax, dim=-1).values  # 快速判断一个向量是否与轴对称包围盒相交

        good_indices = torch.arange(B, device=origins.device)
        grid_mask = torch.ones((gsz[0],gsz[1],gsz[2]), device=origins.device) == 1.0
        grid_mask_isotropyInit = torch.ones((gsz[0],gsz[1],gsz[2]), device=origins.device) == 0.0
        grid_color_isotropyInit = torch.zeros((gsz[0],gsz[1],gsz[2],3), device=origins.device)
        grid_density_init = 0.1 * torch.ones((gsz[0],gsz[1],gsz[2]), device=origins.device)

        # mask1 = t <= tmax # rays intersected with grid
        # mask2 = t < t_d # rays intersected with objects
        # mask = torch.logical_and(mask1, mask2) # 得到与对象相交的光线的索引
        mask = t <= tmax # rays intersected with grid
        good_indices = good_indices[mask]
        good_indices_copy = good_indices.clone()
        origins = origins[mask]
        origins_copy = origins.clone()
        dirs = dirs[mask]
        ...
        # t_d_copy = t_d.clone()
        tmax = tmax[mask]
        # tmax_copy = tmax.clone()

        # t_d[t_d<t] = tmax[t_d<t] # for background
        t_d[t_d<t] = t[t_d<t]
        # t_d_copy[t_d_copy<t_copy] = tmax_copy[t_d_copy<t_copy]
        t_d[t_d>tmax] = tmax[t_d>tmax] # for unrobust depths
        
        '''
        pos = origins + t_d[:, None] * dirs
        pos = pos.clamp_min_(0.0)
        pos[:, 0] = torch.clamp_max(pos[:, 0], gsz_cu[0] - 1)
        pos[:, 1] = torch.clamp_max(pos[:, 1], gsz_cu[1] - 1)
        pos[:, 2] = torch.clamp_max(pos[:, 2], gsz_cu[2] - 1)
        ...
        l.clamp_min_(0)
        lx, ly, lz = l.unbind(-1)
        grid_mask_isotropyInit[lx, ly, lz] = True
        grid_color_isotropyInit[lx, ly, lz, :] = gt # 对同一位置的索引多次赋值，取位置最后的
        '''

        steps = 0
        first_pruning = True # 光线行进的第一个体素是否修剪
        while good_indices.numel() > 0:
            pos = origins + t[:, None] * dirs
            pos = pos.clamp_min_(0.0) # 位于体素网格内(最小)
            # pos[:, 0] = torch.clamp_max(pos[:, 0], gsz[0] - 1)
            # pos[:, 1] = torch.clamp_max(pos[:, 1], gsz[1] - 1)
            # pos[:, 2] = torch.clamp_max(pos[:, 2], gsz[2] - 1)
            pos[:, 0] = torch.clamp_max(pos[:, 0], gsz_cu[0] - 1) 
            pos[:, 1] = torch.clamp_max(pos[:, 1], gsz_cu[1] - 1)
            pos[:, 2] = torch.clamp_max(pos[:, 2], gsz_cu[2] - 1) # 位于体素网格内(最大)
            #  print('pym', pos, log_light_intensity)
            ...
            t_d = t_d[mask]
            # tmax = tmax[mask]
        print(f"Ray marching \033[96m{steps}\033[0m steps!")

        while good_indices_copy.numel() > 0 and steps >=0 and robust_depth:  
            pos_copy = origins_copy + t_copy[:, None] * dirs_copy
            pos_copy = pos_copy.clamp_min_(0.0)

            pos_copy[:, 0] = torch.clamp_max(pos_copy[:, 0], gsz_cu[0] - 1) 
            pos_copy[:, 1] = torch.clamp_max(pos_copy[:, 1], gsz_cu[1] - 1)
            pos_copy[:, 2] = torch.clamp_max(pos_copy[:, 2], gsz_cu[2] - 1)

            l_copy = pos_copy.to(torch.long)
            l_copy.clamp_min_(0)

            lx_copy, ly_copy, lz_copy = l_copy.unbind(-1)

            mask = grid_mask[lx_copy,ly_copy,lz_copy] == True

            grid_mask_isotropyInit[l_copy[mask].unbind(-1)] = True
            grid_color_isotropyInit[l_copy[mask].unbind(-1)] = gt[mask]
            grid_density_init[l_copy[mask].unbind(-1)] = density_init_withDepth # (128.,0.3679), (256.,0.1353), (512.,0.0183)

            good_indices_copy = good_indices_copy[~mask]
            origins_copy = origins_copy[~mask]
            dirs_copy = dirs_copy[~mask]
            gt = gt[~mask]

            t_copy += self.opt.step_size
            t_copy = t_copy[~mask]

            steps -= 1 

        return grid_mask, grid_mask_isotropyInit, grid_density_init, grid_color_isotropyInit # torch.logical_and(grid_mask, grid_mask_isotropyInit)
    ...
    def _volume_render_depth(self, rays: Rays):
        origins = self.world2grid(rays.origins) # ndc to grid for llff
        dirs = rays.dirs / torch.norm(rays.dirs, dim=-1, keepdim=True) # 这里的原点和方向是NDC变换后的
        # viewdirs = dirs
        B = dirs.size(0)
        assert origins.size(0) == B
        gsz = self._grid_size() # max gsz for rendering, not for training
        _scaling = (self._scaling * gsz).to(device=dirs.device) # 该缩放因子用来缩放什么？
        dirs = dirs * _scaling # transform directions from ndc to grid for llff
        delta_scale = 1.0 / dirs.norm(dim=1) # 网格坐标下的单位长度对应于世界坐标系下的长度
        dirs *= delta_scale.unsqueeze(-1) # normalize dirs in grid coordinate frame
        ...
        while good_indices.numel() > 0:
            ...
            l[:, 1] = torch.clamp_max(l[:, 1], (gsz_cu[1] - 2).to(torch.long))
            l[:, 2] = torch.clamp_max(l[:, 2], (gsz_cu[2] - 2).to(torch.long))
            pos -= l # factors for interpolation
            ...
            sigma000, _ = self._fetch_links(links000) # only for depth, not for rgb
            sigma001, _ = self._fetch_links(links001)
            sigma010, _ = self._fetch_links(links010)
            sigma011, _ = self._fetch_links(links011)
            sigma100, _ = self._fetch_links(links100)
            sigma101, _ = self._fetch_links(links101)
            sigma110, _ = self._fetch_links(links110)
            sigma111, _ = self._fetch_links(links111)
            ...
            c0 = c00 * wa[:, 1:2] + c01 * wb[:, 1:2]
            c1 = c10 * wa[:, 1:2] + c11 * wb[:, 1:2]
            sigma = c0 * wa[:, :1] + c1 * wb[:, :1] # bilinear interpolation

            log_att = (
                -self.opt.step_size
                * torch.relu(sigma[..., 0])
                * delta_scale[good_indices] # 这边为什么要乘这个————应该对应的是NDC坐标系中行进了多长距离d=(self.opt.step_size*delta_scale)*1
            )
            weight = torch.exp(log_light_intensity[good_indices]) * (
                1.0 - torch.exp(log_att)
            )
            # depth = weight * (t / torch.norm(_scaling)) 为什么这边用范数？
            depth = weight * t * torch.norm(dirs / _scaling[None, :], dim=-1) # 将grid空间某个方向上的单位长度换算成ndc空间的长度
            # depth = weight * t * delta_scale[good_indices] # 上面一行的语句和这一行的语句其实是等价的————将ndc空间某个方向上的单位长度换算成grid空间的长度
        ...
        # return torch.norm((out_depth[:, None] * dirs_ini) / (self._scaling * gsz).to(device=dirs.device), dim=-1)
        return out_depth
    ...
    def volume_render_image(
        ...
        if self.basis_type != BASIS_TYPE_MLP and imrend_fn_name in _C.__dict__ and not torch.is_grad_enabled() and not return_raylen and use_kernel:
    ...
    def volume_render_depth(self, rays: Rays, sigma_thresh: Optional[float] = None, use_kernel: bool = True):
        ...
        if use_kernel:
            ...
            warn("Using slow volume rendering, should only be used for debugging")
            return self._volume_render_depth(rays)
    ...
    def volume_render_depth_image(
        ...
        all_depths = []
        for batch_start in range(0, camera.height * camera.width, batch_size):
            depths = self.volume_render_depth(rays[batch_start: batch_start + batch_size], sigma_thresh, use_kernel)
    ...
    def upsample(
        ...
        with torch.no_grad():
            ...
            if use_z_order:
                init_links = utils.gen_morton(reso[0], dtype=torch.long).view(-1)
                points[init_links] = points.clone()
            else:
                init_links = torch.arange(n3, device=device, dtype=torch.int32)
            ...
            if use_sphere_bound:
                ...
                sample_vals_density = sample_vals_density[mask]
                sample_vals_sh = sample_vals_sh[mask]
                self.capacity: int = mask.sum()
            ...
            assert self.capacity <= max_elements, \
                f"Total voxels {self.capacity} is more than {max_elements}"
    ...
    def init_dilate(
        ...
        """
        Dilate the grid; used to increase the resolution
        :param reso: int or List[int, int, int], resolution for resampled grid, as in the constructor
        ...
        with torch.no_grad():
            ...
            curr_reso = self.links.shape
            assert reso[0] == curr_reso[0] and reso[1] == curr_reso[1] and reso[2] == curr_reso[2]
            ...
            sample_vals_mask = self.links >= 0 
            
            sample_vals_density = torch.zeros(reso, device=device, dtype=torch.float32)
            sample_vals_density[sample_vals_mask] = self.density_data.data[self.links[sample_vals_mask].to(torch.long)].squeeze()

            sample_vals_sh = torch.zeros((reso[0], reso[1], reso[2], self.sh_data.size(-1)), device=device, dtype=torch.float32)
            sample_vals_sh[sample_vals_mask] = self.sh_data.data[self.links[sample_vals_mask].to(torch.long)]
            
            if dilate:
                sample_vals_mask_ori = sample_vals_mask.clone().view(-1)
                # sample_vals_density_ori = sample_vals_density.clone()
            ...
            sample_vals_sh = sample_vals_sh.view(-1, self.sh_data.size(-1))
            if dilate:
                sample_vals_dilated_mask = torch.logical_and(sample_vals_mask, ~sample_vals_mask_ori)
                sample_vals_density[sample_vals_dilated_mask] = 0.1
    ...
    def resample(
        ...
        use_z_order: bool=False,
        accelerate: bool=True,
        weight_render_stop_thresh: float = 0.0, # SHOOT, forgot to turn this off for main exps..
        max_elements:int=0,
        save_dir: str = None,
        step: int = None
    ):
        ...
        with torch.no_grad():
            ...
            if use_weight_thresh:
                ...
                for i, cam in enumerate(cameras):
                    _C.grid_weight_render(
                        ...
                        0.5, # step_size
                        weight_render_stop_thresh, # maybe be Ti, not Ti*opacity
                ...
                if save_dir is not None:
                    np.save(os.path.join(save_dir, f"max_weight_grid{step}.npy"), max_wt_grid.cpu().numpy())
            ...
            if self.use_gradient_scaling and self.dataset_type == 'nerf':
            # if self.use_gradient_scaling:
                self.gs_grid = self._get_gs_grid(reso, self.dataset_type, self.device)
    ...
    def world2grid(self, points, gsz=None):
        """
