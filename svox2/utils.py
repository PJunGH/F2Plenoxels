...
def convert_to_ndc(origins, directions, ndc_coeffs, near: float = 1.0): # near默认为1合理吗？对于我们的方法（深度引导的体素剪枝）是否可以设为其他值？
    """Convert a set of rays to NDC coordinates."""
    # Shift ray origins to near plane, not sure if needed
    # 这里的变换与原始的NeRF不同，照相机采用的是OpenCV，不是OpenGL
    ...
    o1 = ndc_coeffs[1] * (oy / oz) # 与底层CUDA代码一致
    # o1 = -1. * ndc_coeffs[1] * (oy / oz)
    o2 = 1 - 2 * near / oz

    d0 = ndc_coeffs[0] * (dx / dz - ox / oz)
    d1 = ndc_coeffs[1] * (dy / dz - oy / oz) # 与底层CUDA代码一致
    # d1 = -1. * ndc_coeffs[1] * (dy / dz - oy / oz)
    d2 = 2 * near / oz;
